...
sampler tex2 : register(s1);    //Pass1:デカールマップ
sampler tex3 : register(s3);    //Pass1:デカールマップ
...
float4 pixelShader(VS_OUTPUT In) : COLOR0
{
	...
	//フォグを適応する
	Out = tex2D(tex0, In.Tex)*tex2D(tex2, In.Tex1)* tex2D(tex3, In.Tex2)* In.Col * f + FogColor * (1.0f - f);
