...
{
public:
	Road(D3DXVECTOR3 _startPos, D3DXVECTOR3 _endPos, float _angle,int _nextArray, int _previousArray);
	Road(){}

	~Road();
	virtual void Control(){};
	virtual void Draw(){};
	int GetNextArray(){ return m_NextArray; };
	int GetPreviousArray(){ return m_PreviousArray; };
	virtual bool GetStartOrEndPos(D3DXVECTOR3* _checkPos, D3DXVECTOR3* _outputPos, float* _outputAngleDegree, bool* _startPos);
	virtual D3DXVECTOR3 GetNextPos(){ return D3DXVECTOR3(0,0,0); };
