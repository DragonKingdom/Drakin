...
Road* RoadBuilder::RoadBuild(ROADMANAGER_ENUM::BUILD_TYPE _buildType)
{
	...
	case ROADMANAGER_ENUM::NORMAL:
		pRoad = new NormalRoad(m_StartPos, Vec, angle,m_NextArray,m_PreviousArray);
		break;
	case ROADMANAGER_ENUM::CURVE:
		pRoad = new CurveRoad(m_StartPos, m_ControlPos, Vec, angle, m_NextArray, m_PreviousArray);
		break;
	}
